...
var _ = Describe("ExtauthApiKey", func() {
	...
	It("should create secret without labels", func() {
		err := testutils.GlooctlEE("create secret apikey --name user --namespace gloo-system --apikey secretApiKey")
	...
	It("should create secret with labels", func() {
		err := testutils.GlooctlEE("create secret apikey --name user --namespace gloo-system --apikey secretApiKey --apikey-labels k1=v1,k2=v2")
		Expect(err).NotTo(HaveOccurred())
	...
	It("should error when no name provided", func() {
		err := testutils.GlooctlEE("create secret apikey --namespace gloo-system")
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(Equal(argsutils.NameError))
	})

	It("can print the kube yaml in dry run", func() {
		out, err := testutils.GlooctlEEOut("create secret apikey --name user --namespace gloo-system --apikey secretApiKey --apikey-labels k1=v1,k2=v2 --dry-run")
		Expect(err).NotTo(HaveOccurred())
		fmt.Print(out)
